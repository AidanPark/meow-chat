...
- 프론트엔드는 현재 ReAct-only 모드로 동작하며, 응답을 토큰 단위로 스트리밍합니다.
- MCP 서버는 로컬에서 `bash backend/mcp_servers/start_servers.sh`로 시작할 수 있습니다(소스하지 말고 bash로 실행).
- 스트리밍 관련 오류와 예외 그룹 트레이스는 `frontend/logs/streaming.log`에 기록됩니다.
- 자세한 내용은 `docs/REACT_MODE.md`를 참고하세요.
