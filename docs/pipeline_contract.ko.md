...
이 문서는 OCR 파이프라인의 표준 계약을 정의합니다. 모든 단계는 Pydantic v2 모델 인스턴스를 반환합니다. 직렬화는 경계 지점(API, 로그, UI)에서만 수행하십시오.
...
- stage: str — "ocr", "extraction", "merge" 중 하나
...
- meta
  - tests_len: int — 추출된 검사 항목 수(가능 시)
  - lines_count: int — 고려된 전처리 라인 수(가능 시)
...
- stage: str — envelope stage와 동일(예: "ocr")
...
- 입력: bytes
- 옵션: do_preprocess(bool), preprocess_kwargs(dict)
- 출력: OCRResultEnvelope
- 오류 모드:
  - 잘못된 이미지 바이트: 예외 발생(호출자가 처리)
  - OCR 실패: `data.items=[]`와 가능한 한 최선의 meta로 감싼 envelope 반환
...
# 예시: manager 구현에서 image_to_ocr_stream(...)을 순회
```
