"""
검사결과지(랩 리포트) 추출 파이프라인

이 모듈은 OCR 결과를 받아서 검사 데이터를 추출하고 병합하는 파이프라인을 제공합니다.

주요 기능:
1. OCR 결과 → 검사 데이터 추출 (ocr_to_extraction)
2. 여러 추출 결과 병합 (merge_extractions)
...
사용 예시:
    ...
    extraction = await extractor.ocr_to_extraction(ocr_envelope)
    merged = await extractor.merge_extractions([extraction1, extraction2])

참고:
- 이미지 OCR은 별도 서비스에서 처리하며, 이 클래스는 추출/병합만 담당합니다.
- 의존성(line_preproc, extractor)은 생성자 주입 또는 create_with_deps()로 설정합니다.
"""
...
class LabReportExtractor:
    ...
    def ocr_to_extraction(self, ocr_result: Any, *, progress_cb: ProgressCB = None) -> Envelope[Dict[str, Any], ExtractionMeta]:
        ...
        from src.models.envelopes import Envelope, ExtractionMeta
    ...
    def merge_extractions(self, results: List[Dict[str, Any]], *, progress_cb: ProgressCB = None) -> MergeEnvelope:
        ...
        from src.models.envelopes import MergeData, MergeMeta, MergeEnvelope
        out = MergeEnvelope(
            stage='merge',
