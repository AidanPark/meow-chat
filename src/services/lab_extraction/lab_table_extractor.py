...
class LabTableExtractor:
    ...
    def _resolve_code(self, text: str) -> Optional[str]:
        ...
        구현은 분리된 code_normalizer 모듈에 위임하며,
        모듈이 없을 경우 code_lexicon.resolve_code를 직접 사용한다.

        또한 "SODIUM(Na+)" 같은 형태에서 괄호 앞부분만 추출하여 매칭을 시도한다.
        """
        if not text:
            return None
        ...
        # 1) code_normalizer 모듈이 있으면 사용
        if _resolve_code_norm is not None:
            try:
                result = _resolve_code_norm(text, self.lexicon, ext_resolver=self._ext_resolver)
                if result:
                    return result
            except Exception:
                pass

        # 2) 폴백: code_lexicon.resolve_code 직접 사용
        try:
            from .reference.code_lexicon import resolve_code as _resolve_code_direct
        except ImportError:
            return None

        # 2-1) 전체 텍스트로 먼저 시도
        result = _resolve_code_direct(text, self.lexicon)
        if result:
            return result

        # 2-2) 괄호 앞부분만 추출하여 재시도 (예: "SODIUM(Na+)" -> "SODIUM")
        if '(' in text:
            base = text.split('(')[0].strip()
            if base:
                result = _resolve_code_direct(base, self.lexicon)
                if result:
                    return result

        # 2-3) '-A', '-B' 같은 접미사 제거 후 재시도 (예: "WBC-A" -> "WBC")
        if '-' in text:
            # 마지막 '-' 이후가 단일 문자면 제거
            parts = text.rsplit('-', 1)
